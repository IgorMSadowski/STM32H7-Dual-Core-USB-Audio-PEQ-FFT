...
/* Tamanho total do descritor de configuração USB Audio (em bytes) */
#define USB_AUDIO_CONFIG_DESC_SIZ                     116U
...
/* AUDIO_OUT_PACKET = (sample_rate * nChannels * bits_per_sample) / (8 * 1000) = 288 bytes,
 * 300 proporciona uma margem extra que o PC pode enviar
 * Define o tamanho do pacote
 * */
#define AUDIO_OUT_PACKET                              300U
#define AUDIO_DEFAULT_VOLUME                          70U

/* Define quantos PACKETS de áudio consegue guardar */
#define AUDIO_OUT_PACKET_NUM                          200U

/* Tamanho do buffer de recepção
 * 60000 bytes = 300 * 200, cabe em uint16_t
 *  */
...
typedef struct
{
  ...
  uint8_t buffer[AUDIO_TOTAL_BUF_SIZE + AUDIO_OUT_PACKET]; // <-- ADICIONE ISSO
